...
const PERSONAL_PLAN = {
  ...
  defaultLeverage: Number(process.env.PERSONAL_DEFAULT_LEVERAGE || 6),
  maxLeverage: Number(process.env.PERSONAL_MAX_LEVERAGE || 8),
  minSignalScore: Number(process.env.PERSONAL_MIN_SIGNAL_SCORE || 70)
};
...
// RECALIBRATED: 80 -> 70 (more realistic trigger rate)
...
// ===========================
// RECALIBRATED — wider SL/TP, higher leverage,
// realistic score thresholds for the GLOBAL market score
// (market_cap_change_percentage_24h_usd rarely swings to extremes,
// so 80/20 almost never triggers — 65/35 is much more realistic)
// ===========================
const SL_PCT = Number(process.env.AUTO_TRADE_SL_PCT || 2.0);
const TP_PCT = Number(process.env.AUTO_TRADE_TP_PCT || 5.0);
const AT_LEVERAGE = Number(process.env.AUTO_TRADE_LEVERAGE || 6);
const SCORE_LONG_MIN = Number(process.env.AUTO_TRADE_SCORE_LONG || 65);
const SCORE_SHORT_MAX = Number(process.env.AUTO_TRADE_SCORE_SHORT || 35);
...
bot.command("scandebug", async (ctx) => {
  ...
  try {
    ...
    // ===========================
    // ALSO SHOW GLOBAL SCORE — this is what feeds the main AutoTrade
    // ===========================
    const global = await emergencyTimeout(getGlobal(true), 15000, "CoinGecko global timeout");
    const globalChange = safe(global?.data?.market_cap_change_percentage_24h_usd);
    const globalScore = scoreFromChange(globalChange);
    ...
    return ctx.reply(
      `✅ <b>Scan Debug Complete</b>\n\n` +
        `🌍 <b>Global AutoTrade Score</b>\n` +
        `Score: <b>${globalScore}/100</b>\n` +
        `24h Market Change: <b>${formatPercent(globalChange)}</b>\n` +
        `LONG triggers at: <b>≥${SCORE_LONG_MIN}</b>\n` +
        `SHORT triggers at: <b>≤${SCORE_SHORT_MAX}</b>\n` +
        `Distance to LONG: <b>${Math.max(0, SCORE_LONG_MIN - globalScore)} pts</b>\n` +
        `Distance to SHORT: <b>${Math.max(0, globalScore - SCORE_SHORT_MAX)} pts</b>\n\n` +
        `📡 <b>Personal Scanner</b>\n` +
        ...
        (found.length
          ? `🔥 <b>Top Signals</b>\n${topSignals}`
          : `🧠 No personal scanner signals found.`),
      {
        parse_mode: "HTML",
        disable_web_page_preview: true,
        reply_markup: buildMainKeyboard().reply_markup
...
function detectMarketSetup(coin) {
  ...
  // RECALIBRATED: lower thresholds for more frequent signals
  if (change24h >= 2.5 && volumeToMcap >= 0.025) {
  ...
  if (Math.abs(change24h) >= 1.5 && volumeToMcap >= 0.05) {
...
bot.command("close", async (ctx) => {
  ...
  if (!openPosition) {
    return ctx.reply("⚠️ No tracked AutoTrade position to close.");
  }

  await ctx.reply("⏳ Closing tracked position...");
  await atCloseTrackedPosition("Manual via Telegram");
});
...
bot.command("position", async (ctx) => {
  ...
  if (!openPosition) {
    return ctx.reply("⚠️ No tracked AutoTrade position.", {
      reply_markup: buildMainKeyboard().reply_markup
    });
  }

  const { symbol, side, entryPrice, qty, leverage, ts } = openPosition;

  return ctx.reply(
    `📈 <b>Tracked AutoTrade Position</b>\n\n` +
      `Pair: <b>${escapeHTML(symbol)}</b>\n` +
      `Side: <b>${side === "BUY" ? "LONG" : "SHORT"}</b>\n` +
      `Entry: <b>${formatUsd(entryPrice)}</b>\n` +
      `Qty: <b>${qty}</b>\n` +
      `Leverage: <b>${leverage}x</b>\n` +
      `Opened: <b>${new Date(ts).toLocaleString()}</b>\n\n` +
      `Use /close to exit.`,
    {
      parse_mode: "HTML",
      reply_markup: buildMainKeyboard().reply_markup
...
bot.command("scan", async (ctx) => {
  ...
  await ctx.reply("🧪 Running scanner now...", {
    reply_markup: buildMainKeyboard().reply_markup
  });
...
// Triple confluence: Global Score + BTC Momentum + Scanner
// ===============================
...
const SMART_AT = {
  ...
  confluenceMin:        Number(process.env.AT_CONFLUENCE_MIN             || 3),
  confluenceScoreMin:   Number(process.env.AT_CONFLUENCE_SCORE_MIN       || 70),
...
async function smartEvaluateAndTrade(nextState) {
  try {
    ...
    if (smartAtState.paused) {
      console.log(`[SmartAT] Paused: ${smartAtState.pauseReason}`);
      return;
    }
    ...
    const side   = ev.direction === "LONG" ? "BUY" : "SELL";
    ...
    console.error("[SmartAT] error:", err.message);
  }
}
...
async function smartExecuteAuto(symbol, side, score, ev) {
  try {
    ...
    openPosition = {
      side, symbol, entryPrice: fillPrice,
      qty, slOrderId, tpOrderId,
      score, leverage, riskUsd,
      smartMode: true, ts: Date.now()
    };
    ...
    await sendPrivate(
      ...
      `🛑 SL: <b>${formatUsd(slPrice)}</b> (-${SL_PCT}%)\n` +
      `✅ TP: <b>${formatUsd(tpPrice)}</b> (+${TP_PCT}%)\n` +
      `📊 R/R: <b>1:${(TP_PCT / SL_PCT).toFixed(1)}</b>\n` +
      `Max risk: <b>-$${Number(riskUsd).toFixed(2)}</b>\n` +
      `Target: <b>+$${potentialWin}</b>\n\n` +
      `Use /close to exit manually.\n🌐 wojakmeter.com`
...
function recordSmartTradeResult(won) {
  if (won) {
    smartAtState.consecutiveLosses = 0;
    smartAtState.totalAutoWins++;
  } else {
    smartAtState.consecutiveLosses++;
    smartAtState.totalAutoLosses++;

    if (smartAtState.consecutiveLosses >= SMART_AT.maxConsecutiveLosses) {
      smartAtState.paused      = true;
      smartAtState.pauseReason = `${SMART_AT.maxConsecutiveLosses} consecutive losses`;

      sendPrivate(
        `🧊 <b>Smart AutoTrade PAUSED</b>\n\n` +
        `Reason: <b>${smartAtState.consecutiveLosses} consecutive losses</b>\n\n` +
        `Use /resumeat to resume trading.`
      );
    }
  }
}

bot.command("smartstatus", async (ctx) => {
  if (!isPrivateOwner(ctx)) return replyOwnerOnly(ctx);

  await ctx.reply("🧪 Evaluating signals...");
  ...
  try {
    ...
    const tradingStatus = !autoTradeActive
      ? "🛑 AutoTrade OFF"
      : smartAtState.paused
        ? `🧊 Paused: ${smartAtState.pauseReason}`
        : openPosition
          ? `📈 Position open: ${openPosition.side === "BUY" ? "LONG" : "SHORT"} ${openPosition.symbol}`
          : pendingConfirm
            ? `⏳ Pending: ${pendingConfirm.side} ${pendingConfirm.symbol}`
            : "✅ Ready";
...
bot.command("resumeat", async (ctx) => {
  if (!isPrivateOwner(ctx)) return replyOwnerOnly(ctx);

  smartAtState.paused            = false;
  smartAtState.pauseReason       = null;
  smartAtState.consecutiveLosses = 0;

  return ctx.reply(
    `✅ <b>Smart AutoTrade resumed</b>\n\nConsecutive losses reset to 0.`,
